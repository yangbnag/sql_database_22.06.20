...
CREATE TABLE CHAR_COMPARE (
    sn CHAR(10),
    char_4 CHAR(4), -- 무조건 4글자를 만든다
    char_6 CHAR(6)  -- 무조건 6글자를 만든다.
);
...
FROM char_compare
WHERE sn = '101'
    AND char_4 = char_6 -- 101번이고 char_4와 char_6이 같은가?
;                       -- 공백이 있든 없든 글자만 같으면 true가 된다.
...
FROM char_compare
WHERE sn = '102'
    AND char_4 > char_6 -- SQL[D] > SQL[A] 아스키코드 68 > 65 = TURE
;
...
WHERE sn = '101'
    AND char_4 = varchar_6 -- varchar 는 공백도 포함해서 비교한다.
;
...
FROM varchar_compare
WHERE sn = '101'
    AND char_4 = TRIM(varchar_6) -- TRIM은 문자열 공백제거 함수
;
...
FROM varchar_compare
WHERE sn = '101'
    AND char_4 = 'SQLD         ' -- char로 비교하면 공백 무시하고 비교
;
...
FROM varchar_compare
WHERE sn = '101'
    AND varchar_6 = 'SQLD  ' -- varchar로 비교하면 상수도 varchar 타입으로 놓고 비교
;
...
FROM dual; -- TO_CHAR에서 한글을 쓰려면 한글을 쌍따옴표로 가둬줘야한다.
