...
-- 5. employees테이블에서 manager_id가 101, 102, 103중에 속하는 
  -- 사원의 first_name, salary, manager_id를 조회하세요.
...
-- 6. employees테이블에서 job_id가 'IT_PROG', 'FI_MGR'중에 
-- 포함되지 않는 사원의 first_name, job_id를 조회하세요.
...
-- 10. employees테이블에서 commission_pct가 null이 아닌 사원의 first_name, 
-- job_id, commission_pct를 조회하세요.
...
FROM employees
WHERE 1=1 
AND job_id LIKE '%IT_PROG%' OR salary >= 5000
;
...
FROM employees
WHERE 1=1 
AND (job_id LIKE '%IT_PROG%' OR job_id LIKE '%FI_MGR%')
AND salary >= 5000
;
